...
## Goal
Give Packetry one stable app-side state model before live capture, file ingest, and analyst UI features expand. The foundation contract is one window-scoped controller per workspace, background services behind protocol boundaries, and immutable snapshots handed to SwiftUI.

## Window Ownership
- Every window owns one `PacketryWindowController`.
- The controller publishes a `PacketryWindowSnapshot` value instead of exposing mutable service state directly to the view layer.
...
## Service Boundaries
- `PacketryServiceRegistry` injects app-side dependencies and the `PcapPlusPlusCore` facade.
- `Packetry` talks to `PacketryCoreProviding` only; no app type depends on vendored native wrappers.
- `PacketryBackgroundCoordinator` is an actor that tracks background operations and provides a single cancellation surface for future file-open, decode, and live-capture tasks.
...
- `PacketryWindowSnapshot` is the stable bundle the UI reads and tests assert against.
...
- Shared services may be cached below `PacketryServiceRegistry`, but selected packet state, document state, and onboarding state remain window-scoped.
