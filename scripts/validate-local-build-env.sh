...
# Validates local-only build settings that are intentionally not committed.
is_blank() {
...
if is_blank "${team_id}"; then
    echo "error: Missing TCPVIEWER_DEVELOPMENT_TEAM. Copy .env.example to .env and set your Apple Development Team ID."
    missing=1
...
if is_blank "${build_key}"; then
    echo "error: Missing TCPVIEWER_BUILD_KEY. Copy .env.example to .env and set a private build key."
    missing=1
fi
